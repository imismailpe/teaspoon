...
export default function RecipeCard({ recipe }: { recipe: RecipeType }) {
  ...
  const onShare = () => {
    ...
    if (!navigator.clipboard) {
      alert(
        "Clipboard API not supported in your browser. Please copy the recipe manually."
      );
      return;
    }
    ...
    <>
      <Card>
        <div className="flex w-full justify-between items-center">
          ...
          <button
            onClick={onShare}
            className="text-gray-500 border border-solid px-2 rounded-full shadow-2 bg-orange-400 text-white hover:bg-orange-400 dark:hover:bg-orange-500 cursor-pointer"
          >
            Share
          </button>
        </div>
        <p className="flex w-full justify-between text-gray-500 dark:text-gray-300">
          <span className="">{recipe?.cooking_time}</span>
        ...
        <div className="">
          {recipe?.preparation.map((prep, index) => (
            <div key={index}>{prep}</div>
          ))}
        </div>
      </Card>
      {showNotification ? (
        <div className="fixed bottom-12 right-12 bg-green-600 p-4 rounded-lg shadow-lg text-white">
          Copied to clipboard
        </div>
      ) : null}
    </>
