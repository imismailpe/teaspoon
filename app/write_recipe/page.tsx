...
export default function WriteRecipe() {
  ...
  return (
    <div className='flex flex-col items-center justify-start'>
      <Card>
        <div className='flex flex-col items-center justify-start'>
          <input onChange={e => setDish(e.target.value)} type='text' placeholder='Wish for a dish' className='w-full p-2 border border-gray-300 rounded-md' />
          <button onClick={onSubmitDish} className='mt-4 w-fit bg-blue-500 text-white px-2 rounded-md hover:bg-blue-600'>{processing ? "Loading.." : "Generate Recipe"}</button>
        </div>
      </Card>
      {recipeReady ? (
        <div className='m-auto w-fit'><RecipeCard recipe={recipe} /></div>
      ) : null}
      {showNotification ? <div className='fixed bottom-12 right-12 bg-red-600 text-white p-4 rounded-lg shadow-lg'>
